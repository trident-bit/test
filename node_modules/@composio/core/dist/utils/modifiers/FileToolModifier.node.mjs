...
import { a as ComposioSensitiveFilePathBlockedError, c as logger_default, i as ComposioFileUploadPathNotAllowedError, n as ComposioFileUploadAbortedError, r as ComposioFileUploadError, t as ComposioFileNotFoundError } from "../../FileModifierErrors-C7z16Dfi.mjs";
import { i as transformProperties, n as schemaHasFileDownloadable, r as schemaHasFileUploadable, t as isPlainObject } from "../../FileToolModifier.utils.neutral-B7P7wn-b.mjs";
import { n as getFileDataAfterUploadingToS3, t as downloadFileFromS3 } from "../../fileUtils.node-Dq7ebZPC.mjs";
...
const hydrateFiles = async (value, schema, ctx) => {
	if (schema?.file_uploadable) {
		...
		if (typeof value === "string") {
			...
			logger_default.debug(`Uploading file "${pathOrUrl}"`);
		...
		if (ctx.beforeFileUpload) {
			...
			if (typeof out === "string" && out !== value.name) {
				logger_default.debug(`Uploading file from path "${out}" (replaced File: ${value.name})`);
		...
		logger_default.debug(`Uploading file "${value.name}"`);
...
var FileToolModifier = class {
	...
	async fileUploadModifier(tool, options) {
		...
		try {
			...
			if (error instanceof ComposioSensitiveFilePathBlockedError || error instanceof ComposioFileUploadAbortedError || error instanceof ComposioFileUploadPathNotAllowedError || error instanceof ComposioFileNotFoundError) throw error;
