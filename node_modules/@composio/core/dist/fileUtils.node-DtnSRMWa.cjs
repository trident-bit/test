...
function normalizePathSegments(filePath) {
	...
	try {
		if (node_fs.default.existsSync(absolute)) resolved = node_fs.default.realpathSync(absolute);
	} catch {}
...
function assertSafeFileUploadPath(filePath, options) {
	...
	if (reason) throw new require_FileModifierErrors.ComposioSensitiveFilePathBlockedError(`Refusing to upload: ${reason}. To upload from this path anyway, set sensitiveFileUploadProtection: false on Composio (not recommended) or use a copy outside sensitive locations.`, { meta: {
		filePath,
		reason
	} });
}
...
function tryRealpath(absPath) {
	try {
		if (node_fs.default.existsSync(absPath)) return node_fs.default.realpathSync(absPath);
	} catch {}
	return null;
}
...
function assertPathInsideUploadDirs(filePath, allowlist) {
	...
	if (!real) {
		...
		const parent = node_path.dirname(abs);
		const parentExists = (() => {
			try {
				return node_fs.default.existsSync(parent);
			} catch {
				return false;
			}
		})();
		throw new require_FileModifierErrors.ComposioFileNotFoundError([
	...
	if (allowlist.length === 0) throw new require_FileModifierErrors.ComposioFileUploadPathNotAllowedError([
	...
	for (const dir of allowlist) if (isInsideDir(real, tryRealpath(dir) ?? node_path.resolve(dir))) return;
	throw new require_FileModifierErrors.ComposioFileUploadPathNotAllowedError([
...
const generateTimestampedFilename = (extension, prefix) => {
	return `${prefix || "file_ts"}${Date.now()}${require_buffer.getRandomShortId()}.${extension}`;
};
const readFileContent = async (filePath) => {
	try {
		if (!_platform.platform.supportsFileSystem) throw new Error("File system operations are not supported in this runtime environment");
		const content = _platform.platform.readFileSync(filePath);
		return {
			fileName: generateTimestampedFilename(filePath.split(".").pop() || "txt"),
			content: content instanceof Uint8Array ? require_buffer.uint8ArrayToBase64(content) : require_buffer.uint8ArrayToBase64(new TextEncoder().encode(content)),
...
const readFileContentFromURL = async (path) => {
	...
	let fileName = _platform.platform.basename(pathname);
	...
	return {
		content: require_buffer.uint8ArrayToBase64(content),
		mimeType,
		fileName
	};
};
const uploadFileToS3 = async (fileName, content, toolSlug, toolkitSlug, mimeType, client) => {
	const contentBytes = require_buffer.base64ToUint8Array(content);
	const { key, new_presigned_url: signedURL } = await client.files.createPresignedURL({
		filename: fileName,
		mimetype: mimeType,
		md5: node_crypto.default.createHash("md5").update(contentBytes).digest("hex"),
		tool_slug: toolSlug,
		toolkit_slug: toolkitSlug
	});
	require_FileModifierErrors.logger_default.debug(`Uploading ${key} file to S3: ${key}`);
...
const readFile = async (file) => {
	if (file instanceof File) {
		...
		return {
			fileName: file.name,
			content: require_buffer.uint8ArrayToBase64(new Uint8Array(content)),
			mimeType: file.type
		};
...
const getFileDataAfterUploadingToS3 = async (file, { toolSlug, toolkitSlug, client, sensitiveFileUploadProtection, fileUploadPathDenySegments, fileUploadAllowlist }) => {
	...
	require_FileModifierErrors.logger_default.debug(`Uploading file to S3...`);
	const s3key = await uploadFileToS3(_platform.platform.basename(fileData.fileName), fileData.content, toolSlug, toolkitSlug, fileData.mimeType, client);
	require_FileModifierErrors.logger_default.debug(`Done! File uploaded to S3: ${s3key}`, JSON.stringify(fileData, null, 2));
	return {
...
const getComposioDir = (createDirIfNotExists = false) => {
	try {
		...
		const composioDir = _platform.platform.joinPath(homeDir, require_FileModifierErrors.COMPOSIO_DIR);
		if (createDirIfNotExists && _platform.platform.supportsFileSystem && !_platform.platform.existsSync(composioDir)) _platform.platform.mkdirSync(composioDir);
		return composioDir;
	} catch (_error) {
		return null;
	}
...
const getComposioTempFilesDir = (createDirIfNotExists = false) => {
	try {
		const homeDir = _platform.platform.homedir();
		if (!homeDir) return null;
		const composioFilesDir = _platform.platform.joinPath(homeDir, require_FileModifierErrors.COMPOSIO_DIR, require_FileModifierErrors.TEMP_FILES_DIRECTORY_NAME);
		if (createDirIfNotExists && _platform.platform.supportsFileSystem && !_platform.platform.existsSync(composioFilesDir)) _platform.platform.mkdirSync(composioFilesDir);
...
const saveFile = (file, content, options = {}) => {
	...
	try {
		if (!_platform.platform.supportsFileSystem) {
			require_FileModifierErrors.logger_default.debug("File system operations are not supported in this runtime environment");
			return null;
		}
		let composioFilesDir;
		if (opts.outputDir) {
			const absDir = _platform.platform.joinPath(opts.outputDir);
			try {
				if (!_platform.platform.existsSync(absDir)) _platform.platform.mkdirSync(absDir);
				composioFilesDir = absDir;
			} catch (err) {
				require_FileModifierErrors.logger_default.warn(`fileDownloadDir "${absDir}" is not writable, falling back to the default (~/.composio/files). Error: ${err}`);
		...
		const filePath = _platform.platform.joinPath(composioFilesDir, _platform.platform.basename(file));
		require_FileModifierErrors.logger_default.info(`Saving file to: ${filePath}`);
		if (content instanceof Uint8Array) _platform.platform.writeFileSync(filePath, content);
		else _platform.platform.writeFileSync(filePath, content, "utf8");
		return filePath;
	} catch (_error) {
		require_FileModifierErrors.logger_default.debug(`Error saving file: ${_error}`);
		return null;
	}
};

//#endregion
Object.defineProperty(exports, 'downloadFileFromS3', {
  enumerable: true,
  get: function () {
    return downloadFileFromS3;
  }
});
Object.defineProperty(exports, 'getFileDataAfterUploadingToS3', {
  enumerable: true,
  get: function () {
