	...
	*/
	static async sendMetric(payload) {
		try {
			...
			require_FileModifierErrors.logger_default.debug("Error sending metric telemetry", error);
		}
	}
	/**
	...
	*/
	static async sendErrorLog(payload) {
		try {
			...
			require_FileModifierErrors.logger_default.debug("Error sending error telemetry", error);
		}
	}
};
...
var TelemetryTransport = class {
	...
	batchProcessor = new BatchProcessor(200, 10, async (data) => {
		require_FileModifierErrors.logger_default.debug("Sending batch of telemetry metrics", data);
	...
	setup(metadata) {
		...
		this.telemetrySource = {
			...
			environment: require_FileModifierErrors.getEnvVariable("NODE_ENV", "production")
		};
	...
	*/
	shouldSendTelemetry() {
		...
		const nodeEnv = (require_FileModifierErrors.getEnvVariable("NODE_ENV", "development") || "").toLowerCase();
		const isDisabledEnvironment = telemetryDisabledEnvironments.includes(nodeEnv);
		const isTelemetryDisabledByEnv = require_FileModifierErrors.getEnvVariable("TELEMETRY_DISABLED", "false") === "true";
	...
	*/
	async prepareAndSendErrorTelemetry(error, instrumentedClassName, name, startTime, durationMs) {
		...
		else if (error instanceof require_FileModifierErrors.ComposioError) telemetryPayload.error = {
			errorId: error.errorId,
			name: error.name,
			code: error.code,
			message: error.message,
	...
	*/
	async sendMetric(payload) {
		if (!this.shouldSendTelemetry()) {
			require_FileModifierErrors.logger_default.debug("Telemetry is disabled, skipping metric telemetry", payload);
			return;
		}
		try {
			require_FileModifierErrors.logger_default.debug("SDK Metric", payload);
			await TelemetryService.sendMetric(payload);
		} catch (error) {
			require_FileModifierErrors.logger_default.error("Error sending metric telemetry", error);
		}
	}
	async sendErrorTelemetry(payload) {
		...
		try {
			require_FileModifierErrors.logger_default.debug("SDK Error Telemetry", payload);
			await TelemetryService.sendErrorLog(payload);
		} catch (error) {
			require_FileModifierErrors.logger_default.error("Error sending error telemetry", error);
		}
	}
	/**
	...
	registerExitHandlers() {
		...
		const flushSync = () => {
			this.flush().catch((error) => {
				require_FileModifierErrors.logger_default.debug("Error flushing telemetry on exit", error);
			});
		};
		...
		const createSignalHandler = (signal) => {
			const handler = () => {
				require_FileModifierErrors.logger_default.debug(`Received ${signal}, flushing telemetry...`);
				this.flush().catch((error) => {
					require_FileModifierErrors.logger_default.debug("Error flushing telemetry on signal", error);
				}).finally(() => {
